...
"                num_bg_threads=0, bg_work=1000, num_locks=1, /)\n"
"--\n"
"\n");
...
_testinternalcapi_benchmark_locks(PyObject *module, PyObject *const *args, Py_ssize_t nargs)
{
    ...
    Py_ssize_t num_bg_threads = 0;
    Py_ssize_t bg_work = 1000;
    Py_ssize_t num_locks = 1;

    if (!_PyArg_CheckPositional("benchmark_locks", nargs, 1, 9)) {
        goto exit;
    }
    {
        Py_ssize_t ival = -1;
    ...
    {
        Py_ssize_t ival = -1;
        PyObject *iobj = _PyNumber_Index(args[8]);
        if (iobj != NULL) {
            ival = PyLong_AsSsize_t(iobj);
            Py_DECREF(iobj);
        }
...
skip_optional:
    return_value = _testinternalcapi_benchmark_locks_impl(module, num_threads, work_inside, work_outside, time_ms, num_acquisitions, total_iters, num_bg_threads, bg_work, num_locks);
