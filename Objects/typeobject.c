...
static PyObject *
type_lookup_ref_cache_miss(PyTypeObject *type, PyObject *name, unsigned int *version,
...
_PyType_LookupRefAndVersion(PyTypeObject *type, PyObject *name, unsigned int *version)
{
    ...
    if (result.hit) {
        ...
        return Py_XNewRef(result.value);
    }

    return type_lookup_ref_cache_miss(type, name, version, mro_cache);
}
